...
The project supports optional real PQC libraries when they are installed, and
ships with a browser-friendly hash-based WOTS-style signature bundle so the
web app can verify signatures without storing any client secret on the server.
"""
...
class QuantumSafeSignature:
    ...
    - Browser flow: WOTS-SHA256 key bundles, which are hash-based and verifiable
      with public key material only.
    - Optional Python flow: ML-DSA, SPHINCS+, or XMSS when corresponding runtime
      libraries are installed.
    """

    SIGNATURE_ALGORITHM = WOTS_ALGORITHM
    NONCE_LENGTH = 32
    ...
    def _init_backend(cls) -> None:
        ...
        if OQS_AVAILABLE:
            cls._backend_algorithm = "ML-DSA-65"
        elif SPHINCS_AVAILABLE:
            cls._backend_algorithm = "SPHINCS+-SHA2-256f"
        elif XMSS_AVAILABLE:
    ...
    def get_supported_algorithms(cls) -> List[Dict[str, object]]:
        algorithms: List[Dict[str, object]] = [
            {
                "name": WOTS_ALGORITHM,
                "type": "hash-based one-time signature bundle",
                "available": True,
        ...
        algorithms.extend([
            {
                "name": "ML-DSA-65",
                "type": "lattice-based NIST signature",
                "available": OQS_AVAILABLE,
    ...
    def generate_keypair(slots: int = DEFAULT_WOTS_SLOTS) -> Tuple[str, str]:
        ...
        The web prototype uses WOTS-SHA256 because it works in browsers and can
        be verified by the server using only public key material. Optional real
        PQC libraries are still detected and reported for Python-side extension.
        """
    ...
    def _verify_optional_pqc(message: str, signature: str, public_key: str, algorithm: str) -> bool:
        try:
            ...
            if algorithm == "SPHINCS+-SHA2-256f" and SPHINCS_AVAILABLE:
    ...
    def inspect_public_key(public_key: str) -> Dict[str, object]:
        ...
        if algorithm not in {"ML-DSA-65", "SPHINCS+-SHA2-256f", "XMSS-SHA2_10_256"}:
...
print(f"[CRYPTO] Browser-compatible default: {WOTS_ALGORITHM}\n")
